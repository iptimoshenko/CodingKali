...
def equal(A, B):
    ...
    """
    assert A.D == B.D
    if A.f.keys() == B.f.keys():
        return sum([A[i,j] == B[i,j] for (i,j) in A.f.keys()]) == len(A.f)
    else:
        return False
...
def add(A, B):
    ...
    for k in B.f.keys():
        if k not in elist:
            elist.append(k)
    return Mat(A.D, {(i,j): A[i,j] + B[i,j] for (i,j) in elist})
...
def scalar_mul(M, x):
    ...
    True
    """
    return Mat(M.D, {(i, j): M[i,j]*x for i,j in M.f.keys() if M[i,j]*x!=0})
...
def transpose(M):
    ...
    True
    """
    return Mat((M.D[1], M.D[0]), {(j, i): getitem(M, (i, j)) for (i, j) in M.f.keys()})
...
def vector_matrix_mul(v, M):
    ...
    """
    assert M.D[0] == v.D
    return Vec(M.D[1], {k: sum([v[d]* M[d,k] for d in v.D if M[d,k]!=0 if v[d]!=0]) for d in v.D for k in M.D[1] if sum([v[d]* M[d,k] for d in v.D if M[d,k]!=0 if v[d]!=0])!=0})
...
def matrix_vector_mul(M, v):
    ...
    """
    assert M.D[1] == v.D
    return Vec(M.D[0], {k: sum([M[k, d]*v[d] for d in v.D if M[k,d]!=0 if v[d]!=0]) for k in M.D[0]
                        if sum([M[k, d]*v[d] for d in v.D if M[k,d]!=0 if v[d]!=0])!=0})
...
def matrix_matrix_mul(A, B):
    ...
    """
    assert A.D[1] == B.D[0]
    return Mat((A.D[0], B.D[1]), {(i,j): sum([A[i,d] * B[d, j] for d in A.D[1]])
            for i in A.D[0] for j in B.D[1] if sum([A[i,d] * B[d, j] for d in A.D[1]])!=0 }) #
